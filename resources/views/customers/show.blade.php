<x-layout>
    <div class="mb-6 flex justify-between items-center">
        ...
        <div class="space-x-3">
            <a href="{{ route('customers.edit', $customer) }}" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-md">Edit Customer</a>
    ...
    <x-panel class="p-6 max-w-3xl mx-auto space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-b pb-4">
            ...
            <div>
                ...
                <p class="text-lg text-gray-800 mt-1">{{ $customer->email }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-b pb-4">
            <div>
                ...
                <p class="text-base text-gray-800 mt-1">{{ $customer->phone }}</p>
            </div>
            <div>
                <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">City</span>
                <p class="text-base text-gray-800 mt-1">{{ $customer->city ?? '-' }}</p>
            </div>
        </div>

        <div class="border-b pb-4">
            <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">Address</span>
            <p class="text-base text-gray-800 mt-1">{{ $customer->address ?? '-' }}</p>
        </div>

        <div>
            <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">Notes</span>
            <p class="text-sm text-gray-700 mt-1 bg-gray-50 p-3 rounded border border-gray-100">{{ $customer->notes ?? 'No notes provided.' }}</p>
        </div>
    </x-panel>
</x-layout>