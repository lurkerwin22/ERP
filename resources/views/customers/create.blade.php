<x-layout>
    ...
    <x-panel class="p-6 max-w-3xl mx-auto">
        <x-forms.form action="{{ route('customers.store') }}" method="POST">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-forms.input label="Full Name *" name="name" required />
                <x-forms.input label="Email *" name="email" type="email" required />
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <x-forms.input label="Phone Number *" name="phone" required />
